pipeline {
    ...
    parameters {
        choice(name: "service", choices: ["bank-app", "front-ui", "account-service", "blocker-service", "cash-service", "exchange-service", "exchange-generator-service", "notification-service", "transfer-service", "identity-provider", "postgre-db", "zipkin", "prometheus", "grafana", "logstash", "elasticsearch"], description: "Choose service for build and deploy")
...
def isNeedBuild(serviceName, Closure steps) {
    if (serviceName == 'postgre-db' || serviceName == 'identity-provider' || serviceName == 'zipkin' || serviceName == 'prometheus' || serviceName == 'grafana' || serviceName == 'logstash' || serviceName == 'elasticsearch') {
